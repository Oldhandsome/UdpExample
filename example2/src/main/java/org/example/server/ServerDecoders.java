...
class ServerDecoders {

    private final static AtomicInteger socketIdGenerator;

    private final static Map<Integer, ChannelHandlerContext> contextHolder = new ConcurrentHashMap<>();

    static {
        Random random = new Random();
        socketIdGenerator = new AtomicInteger(random.nextInt(1000));
    }
            ...
            if (msg instanceof ControlPacket controlPacket) {
                ...
                switch (controlPacketType) {
                    case ControlPacket.CONTROL_PACKET_TYPE_HANDSHAKE: {
                        ...
                        int clientStartSequence = control.readInt();
                        int clientConnectionType = control.readInt();
                        int clientSocketId = control.readInt();

                        logger.debug("client connection type: {}", clientConnectionType);


                        if(clientConnectionType == ControlPacket.HAND_SHAKE_TYPE_REGULAR_CONNECTION_REQUEST){
                            int socketId = socketIdGenerator.incrementAndGet();

                            logger.debug("client socket id: {}", socketId);

                            contextHolder.put(socketId, ctx);

                            ControlPacket packet = ControlPacket.handShake(
                                    ServerEncoders.getCurrentSequenceId(),
                                    ControlPacket.HAND_SHAKE_TYPE_RESPONSE_1,
                                    socketId
                            );
