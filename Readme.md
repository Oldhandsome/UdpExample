...
> 注意：组播的端口设置
> 
> UDP组播是一种多点广播技术，可以将一份数据发送给多个接收者，这些接收者可以处于不同的子网中。UDP组播采用IP地址段作为多播地址，允许多个主机共享同一个IP地址。接收端通过加入组播组，即加入指定的IP地址段，来接收发送端发送的数据。
> 接收端绑定的端口用于控制接收，指定接收UDP数据包的端口号。在接收端加入组播组后，发送端发送数据包时需要指定组播地址和端口号，只有当发送端使用的端口号和接收端绑定的端口号匹配，接收端才能接收到数据包。
> 如果发送端发送数据包时指定的端口号与接收端绑定的端口号不同，那么接收端是不会接收到数据包的。也就是说，即使发送端向已经加入组播组的地址发送数据包，只要发送的端口号与接收端绑定的端口号不匹配，接收端也不会接收到数据包，也不会占用接收端的网络接口带宽。
> 
> 
> 
